              ...
              data: (farmers) {
                return productsAsync.when(
                  data: (products) {
                    ...
                    // Week 1 (Current week)
                    final w1Start = now.subtract(Duration(days: now.weekday - 1));
                    final w1End = w1Start.add(const Duration(days: 6, hours: 23, minutes: 59));
                    final w1Farmers = farmers.where((f) => f.createdAt.isAfter(w1Start)).length;
                    final w1Products = products.where((p) => p.createdAt.isAfter(w1Start)).length;

                    // Week 2 (Previous week)
                    final w2Start = w1Start.subtract(const Duration(days: 7));
                    final w2End = w2Start.add(const Duration(days: 6, hours: 23, minutes: 59));
                    final w2Farmers = farmers.where((f) => f.createdAt.isAfter(w2Start) && f.createdAt.isBefore(w2End)).length;
                    final w2Products = products.where((p) => p.createdAt.isAfter(w2Start) && p.createdAt.isBefore(w2End)).length;

                    // Week 3 (Two weeks ago)
                    final w3Start = w2Start.subtract(const Duration(days: 7));
                    final w3End = w3Start.add(const Duration(days: 6, hours: 23, minutes: 59));
                    final w3Farmers = farmers.where((f) => f.createdAt.isAfter(w3Start) && f.createdAt.isBefore(w3End)).length;
                    final w3Products = products.where((p) => p.createdAt.isAfter(w3Start) && p.createdAt.isBefore(w3End)).length;

                    // Monthly (Current month)
                    final mStart = DateTime(now.year, now.month, 1);
                    final mFarmers = farmers.where((f) => f.createdAt.isAfter(mStart)).length;
                    final mProducts = products.where((p) => p.createdAt.isAfter(mStart)).length;
